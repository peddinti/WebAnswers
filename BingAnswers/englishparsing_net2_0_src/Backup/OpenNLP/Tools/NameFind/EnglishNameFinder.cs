...
namespace OpenNLP.Tools.NameFind
{
	...
	public class EnglishNameFinder 
	{
		...
		private Dictionary<T, string>[] CreatePreviousTokenMaps<T>(string[] finders)
		{
            Dictionary<T, string>[] previousTokenMaps = new Dictionary<T, string>[finders.Length];
			for (int currentFinder = 0, finderCount = finders.Length; currentFinder < finderCount; currentFinder++)
			{
                previousTokenMaps[currentFinder] = new Dictionary<T, string>();
			}
			return previousTokenMaps;
		}

        private void ClearPreviousTokenMaps<T>(Dictionary<T, string>[] previousTokenMaps)
		{
        ...
        private void UpdatePreviousTokenMaps<T>(Dictionary<T, string>[] previousTokenMaps, T[] tokens, string[][] finderTags)
		{
			for (int currentMap = 0, mapCount = previousTokenMaps.Length; currentMap < mapCount; currentMap++)
			{
		...
		private string ProcessParse(string[] models, Parse lineParse)
		{
			...
			Dictionary<Parse, string>[] previousTokenMaps = CreatePreviousTokenMaps<Parse>(models);

			Parse[] tokens = lineParse.GetTagNodes();
			...
			UpdatePreviousTokenMaps<Parse>(previousTokenMaps, tokens, finderTags);
			for (int currentFinder = 0, finderCount = models.Length; currentFinder < finderCount; currentFinder++)
			{
				int start = -1;
				...
				AddNames(models[currentFinder], names, tokens, lineParse);
			}
		...
		private string ProcessText(string[] models, string line)
		{
			...
			string[][] finderTags = new string[models.Length][];
			Dictionary<string, string>[] previousTokenMaps = CreatePreviousTokenMaps<string>(models);
			
			if (line.Length == 0)
			{
				ClearPreviousTokenMaps<string>(previousTokenMaps);
				output.Append("\r\n");
			}
			else
			{
				...
				UpdatePreviousTokenMaps<string>(previousTokenMaps, tokens, finderTags);
				for (int currentToken = 0, tokenCount = tokens.Length; currentToken < tokenCount; currentToken++)
				{
					for (int currentFinder = 0, finderCount = models.Length; currentFinder < finderCount; currentFinder++)
					{
